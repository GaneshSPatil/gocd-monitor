...
module Gocd
  module Monitor
    class ApiSupport
      ...
      def gather_os_facts(dirname)
        options.logger.info 'Gathering facts about this machine'
        File.open(File.join(dirname, "os-facts.json"), 'w') {|f|
          f.puts JSON.pretty_generate(Facter.to_hash)
        }
      end

      def create_persistent_connection(options)
      ...
      def log_info_to_file(server_info, dirname)
        time = Time.now.strftime('%Y-%m-%d_%H:%M:%S%z')
        filename = File.join(dirname, time)
      ...
      def perform
        ...
        dirname = File.join(options.dest_dir, 'api-support')
        begin_time = Time.now
        FileUtils.mkdir_p(dirname)
        conn = create_persistent_connection(options)
        gather_os_facts(dirname)
        while true
          begin
            options.logger.debug('Making Request To Get GoCD Server Information')
            server_info = Gocd::Monitor::ApiSupport.new(options).get_server_info(conn)
            log_info_to_file(server_info, dirname)
          rescue => e
